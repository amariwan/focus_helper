...
while True:
    ...
    if True:
        ...
        if keys == ord('o'):
            ...
            if not os.path.exists('/home/pi/Bilder/calibration/focus_results/' + name[0]):
                os.mkdir('/home/pi/Bilder/calibration/focus_results/' + name[0])
            if not os.path.exists('/home/pi/Bilder/calibration/focus_results/' + name[0] + '/' +name[1]+'_'+name[2]):
                os.mkdir('/home/pi/Bilder/calibration/focus_results/' + name[0] + '/' +name[1]+'_'+name[2])


            
            cv2.imwrite('/home/pi/Bilder/calibration/focus_results/' + name[0] + '/' +name[1]+'_'+name[2]  + "/focus_" + time.strftime("%Y%m%d-%H%M%S") + sys.argv[1] + '.jpg',frame)
            #cv2.imwrite('/home/pi/Bilder/calibration/focus_results/' + "edges_" + time.strftime("%Y%m%d-%H%M%S") +'.jpg',edges)
            # opening a file in 'w'
            file = open('/home/pi/Bilder/calibration/focus_results/' + name[0] + '/' +name[1]+'_'+name[2]  + '/' + sys.argv[1]+ '_' +time.strftime("%Y%m%d-%H%M%S") +'.txt', 'w')
            # write() - it used to write direct text to the file
            # writelines() - it used to write multiple lines or strings at a time, it takes iterator as an argument
            # writing data using the write() method
            file.write("Upper left: " + str(white_pixels1)+"    "+"Max: "+str(tmp1)+" Time: "+str(count1) + "\nUpper right: " + str(white_pixels2)+"    "+"Max: "+str(tmp2)+" Time: "+str(count2) +"\nLower left: " +str(white_pixels3)+"    "+"Max: "+str(tmp3)+" Time: "+str(count3)+ "\nLower right: " +str(white_pixels4)+"    "+"Max: "+str(tmp4)+" Time: "+str(count4)+ "\nMid: " +str(white_pixelsmid)+"    "+"Max: "+str(tmpmid)+" Time: "+str(countmid))
            ...
            sleep(1)
                ...
                #output = strftime("/home/pi/Shared/image-%d%m%Y_%H%M%S.jpg")
            output = time.strftime('/home/pi/Bilder/calibration/focus_results/' + name[0] + '/' +name[1]+'_'+name[2] + '/image-%d%m%Y_%H%M%S_' + sys.argv[1] + '.jpg')
            camera = PiCamera(resolution = (3280,2464))
            #camera.ISO = 100
            camera.led = False
            #camera.framerate = 25
